# user enters wake up timing, wants to check what time to sleep
...
def main_calculation(user_time, rem_sleep_timings):
    ...
    for time in rem_sleep_timings:
        ...
        # execute minus timings
        minutes = user_minutes - rem_minutes - sleep_prep_time
        overflow_hour, minutes = handle_negative_minutes(minutes)
        hours = user_hours - rem_hours - overflow_hour
        hours = handle_negative_hours(hours)
...
def handle_negative_minutes(minutes):
    overflow_hour = 0
    if minutes < 0:
        overflow_hour = 1
        minutes = minutes + 60
...
def handle_negative_hours(hours):
    if hours < 0:
        return hours + 24
    return hours
